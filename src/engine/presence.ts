...
export function computePresenceState(
  ...
  if (forcedWake) {
    ...
    if ((busy.checkAfterMin ?? 15) <= 5) {
      ...
      if (inActiveDialog) {
        hint += `\n\nВАЖНО: У вас был активный диалог, но у тебя только что началось занятие (${busy.label}). ПРЕДУПРЕДИ его об уходе (например, "я по делам, позже напишу" или "я на пару, потом отвечу"). Стиль предупреждения зависит от уровня ваших отношений и твоей persona.`;
      }
    }
  } else if (online) {
    if (inActiveDialog) {
